...
优先读取**暂存区**的变更；若暂存区为空，则读取**工作区（工作树）**的变更。
...
如果暂存区和工作区都没有任何变更，告知用户"当前没有可提交的改动"并终止。
...
**中文翻译说明**：一段简洁的中文，帮助用户快速理解 commit 内容。不要超过两行。

**英文 commit message**：完全依照项目 git log 中观察到的格式。如果无法从 git log 推断出明确风格，使用以下推荐格式：
